...
class Schemas extends EventEmitter {
  ...
  constructor (options = {}) {
     ...
     * @type {Ajv}
     */
    this.validator = new Ajv({ ...ajvOptions, useDefaults: true })

    /**
     * Ajv instance without useDefaults (strict validation, no defaults applied)
     * @type {Ajv}
     */
    this.validatorStrict = new Ajv({ ...ajvOptions })
  ...
  addStringFormats (formats) {
    Object.entries(formats).forEach(([name, re]) => {
      ...
      this.validatorStrict.addFormat(name, format)
    })
  }
  ...
  addKeyword (definition, options = {}) {
    try {
      for (const v of [this.validator, this.validatorStrict]) {
  ...
  async createSchema (filePath, options = {}) {
    const schema = new Schema({
      ...
      validatorStrict: this.validatorStrict,
