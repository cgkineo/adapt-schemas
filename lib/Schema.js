...
class Schema extends EventEmitter {
   ...
   */
  constructor ({ enableCache, filePath, validator, validatorStrict, xssWhitelist, schemaLibrary }) {
    super()
    ...
    /**
     * Compiled validation function (applies defaults during validation)
     * @type {function}
     */
    this.compiled = undefined

    /**
     * Compiled validation function (strict, no defaults applied)
     * @type {function}
     */
    this.compiledStrict = undefined
     ...
     * @type {Ajv}
     */
    this.validatorStrict = validatorStrict
  ...
  async build (options = {}) {
    ...
    if (options.compile !== false) {
      ...
      this.compiledStrict = await this.validatorStrict.compileAsync(built)
    }
  ...
  validate (dataToValidate, options = {}) {
    ...
    let compiled = opts.useDefaults ? this.compiled : this.compiledStrict

    if (!compiled) {
      this.emit('warning', `No compiled function for ${this.name}, compiling now`)
      const v = opts.useDefaults ? this.validator : this.validatorStrict
