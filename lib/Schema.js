...
class Schema extends EventEmitter {
  ...
  validate (dataToValidate, options = {}) {
    ...
    const data = this.applyDefaults(_.cloneDeep(dataToValidate), opts.useDefaults ? this.getObjectDefaults() : {})
  ...
  /**
   * Deep merge that applies defaults to nested objects without overwriting
   * existing values. Unlike _.defaultsDeep, arrays are treated as atomic
   * values (replaced, not merged element-by-element).
   * @param {Object} data The data to apply defaults to (mutated in place)
   * @param {Object} defaults The defaults object from getObjectDefaults()
   * @returns {Object} The data with defaults applied
   */
  applyDefaults (data, defaults) {
    for (const key in defaults) {
      if (data[key] === undefined) {
        data[key] = defaults[key]
      } else if (_.isPlainObject(data[key]) && _.isPlainObject(defaults[key])) {
        this.applyDefaults(data[key], defaults[key])
      }
    }
    return data
  }
